...
- **后续运行**：打开启动中心，选择桌面 GUI、无头 WebUI 或纯命令行完整核心。

启动方式：

- **桌面 GUI**：打开桌面窗口，先进入 WebUI 管理界面，可在窗口内启动核心。
- **无头 WebUI**：不打开桌面窗口，只在命令行托管 WebUI，可从浏览器启动/停止核心。
- **纯命令行完整核心**：沿用旧启动行为，直接启动完整核心与 WebUI 服务。
