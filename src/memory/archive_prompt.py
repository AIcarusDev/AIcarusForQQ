...
- "roles"：每个 "event" 至少要有一个填 "entity" 的 "role"：
  - "entity" 命名：你自己=`Bot:self`；用户=`User:qq_{{id}}`；工具/产品=`Tool:qwen`；公众人物=`Person:马斯克`；组织=`Org:OpenAI`；抽象内容/文本进 "value_text"。
  - 群聊识别：`<sender id="123" nickname="昵称"/>` → `User:qq_123`；`<sender id="self"/>` → `Bot:self`；严禁写纯昵称或裸 `User`（会导致多人错误合并）
...
- "summary"：须是一句含主谓宾的完整句（动作须有目标对象）；即便发言看起来是普适表述，若它出现在特定话题的讨论中，也需要注明话题语境；以「昵称#qq_{{id}}」称呼对方；提及自己时直接用「我」;反事实/假设需注明假设语境；谨慎处理双关/梗等他义。
...
=== 黄金规则 (违反会被静默丢弃) ===
1. 涉及「教/学/讲解/纠正/问/反驳/答应/拒绝/承诺/约定」的句子必须用 event, 且: agent=实施动作的人 / recipient=听众 / theme=内容。
   告知/讲解知识或规则时用 `teach`；只是转述普通信息时用 `say`，不要使用 `tell`。
   ...
   正例(对): event_type='teach', roles=[{role:'agent',entity:'User:qq_123'},
   {role:'recipient',entity:'Bot:self'},{role:'theme',value_text:'X'}]
2. 否定语义用 event_type 表达 (如 dislike/refuse), 不要人工造一个「不喜欢」谓词。
...
9. **三方对话**: A 跟 B 交流不涉及 Bot 的场景仍要记录, agent=A 的 `User:qq_{Aid}`, recipient=B 的 `User:qq_{Bid}`,
   ...
   例: 晓晓 对 阿明 说「等会帮你修掉」→ recipient=User:qq_{阿明id}（不是Bot:self）。
...
11. **摘要自洽原则**: summary 和 value_text 必须**脱离原始对话独立可读**，不能依赖读者记得上下文。
