...
- entity 命名：Bot 自己=`Bot:self`；用户=`User:qq_{id}`；工具/产品=`Tool:qwen`；公众人物=`Person:马斯克`；组织=`Org:OpenAI`；抽象内容/文本进 value_text
...
"confidence"：参考分界值与相关介绍，填写最接近的小数
...
3. 假设/反事实用 modality='hypothetical'。
4. 会随时间变化的事实 (年龄/状态/今天的天气/正在做某事) 用 event_type 描述动作, ctx=episodic。
5. 你说的话也标 context_type='episodic'。
