...
def schedule_compression_archive(summary_text: str, coverage_end_seq: int) -> None:
    """Schedule long-term memory extraction from a compressed context summary."""
...
def _build_compression_archive_dialogue(
    *,
    ...
    parts = [
        ...
        (
            "Extract only durable long-term memories from this compressed context "
            "summary. Do not archive ordinary chat turns, image sends, one-word "
            "reactions, or facts whose only value is 'someone said X'. The raw "
            "chat_messages table is the source of truth for who-said-what; this "
            "archive should contain stable preferences, identities, commitments, "
            "important events, project states, relationships, and repeatedly "
            "confirmed facts. Resolve nicknames with member_aliases when possible."
        ),
        ...
        for event in events_in:
            ...
            try:
                ...
                event_id = await _db_write_event(
                    ...
                    reason=reason or (
                        "extracted from compressed context summary"
                        if archive_mode == "compression_summary"
                        else "从对话中自动提取"
                    ),
                    conv_type=conv_type,
