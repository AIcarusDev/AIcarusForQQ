...
from .config import runtime_adapter_config
from .conversation import make_session_key, parse_session_key
...
def schedule_history_recovery(client) -> None:
    ...
    qq_cfg = getattr(runtime, "config", {}) if runtime is not None else {}
    cfg = _load_config((qq_cfg or {}).get("recovery", {}) or {})
    if not cfg.enabled:
...
def _normalize_target(session_key: str, conv_type: str, conv_id: str, conv_name: str = "") -> RecoveryTarget:
    ...
    if not norm_type:
        norm_type, parsed_id = parse_session_key(session_key)
        norm_id = norm_id or parsed_id
    return RecoveryTarget(
        session_key=make_session_key(norm_type, norm_id) if norm_type and norm_id else session_key,
        conv_type=norm_type,
        conv_id=norm_id,
...
async def _build_targets(cfg: RecoveryConfig) -> list[RecoveryTarget]:
    ...
    runtime = get_platform("qq")
    qq_adapter_cfg = runtime_adapter_config(getattr(runtime, "config", {}) if runtime is not None else {})
    if cfg.seed_from_whitelist and is_whitelist_mode_enabled(qq_adapter_cfg):
        for user_id in get_whitelist_ids(qq_adapter_cfg, "private"):
            _merge(_normalize_target(make_session_key("private", user_id), "private", user_id))
        for group_id in get_whitelist_ids(qq_adapter_cfg, "group"):
            group_name, _, _ = await get_group_info(group_id)
            _merge(_normalize_target(make_session_key("group", group_id), "group", group_id, group_name))
...
async def _persist_messages(
    ...
    *,
    ...
    for message in messages:
        ...
        entry = await qq_events.qq_adapter_event_to_context(
            ob11_message,
        ...
        if inject_recent:
            await qq_events.download_pending_images(entry)
            await qq_events.expand_forward_previews(entry, client)
        else:
