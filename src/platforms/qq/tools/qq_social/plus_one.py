...
def make_handler(qq_client: Any, qq_session_provider: Callable[[], Any | None]) -> Callable:
    ...
    def execute(message_id: str | int, **kwargs) -> dict:
        ...
        content_text = qq_segments.qq_adapter_segments_to_text(
            segments,
            bot_id=session._qq_id,
            bot_display_name=session._qq_name,
        )
        content_segs = qq_segments.build_content_segments(
            segments,
            bot_id=session._qq_id,
            bot_display_name=session._qq_name,
        )
        content_type = qq_segments._determine_content_type(segments)
