...
def make_handler(qq_client: Any, config: dict) -> Callable:
    ...
    def execute(**kwargs) -> dict:
        ...
        if needs_adapter and (not qq_client or not qq_client.connected):
            return {"error": "QQ adapter 未连接，无法获取列表"}

        loop: asyncio.AbstractEventLoop | None = (
            getattr(qq_client, "_loop", None)
        ...
        qq_adapter_cfg: dict = get_qq_adapter_config(config)
        result: dict = {}
        ...
        if query_type in (None, "friend"):
            try:
                raw_friends: list[dict] | None = run_coroutine_sync(
                    qq_client.send_api("get_friend_list", {}),
                    loop,
                    timeout=15,
                )
        ...
        if query_type in (None, "group"):
            try:
                raw_groups: list[dict] | None = run_coroutine_sync(
                    qq_client.send_api("get_group_list", {}),
                    loop,
                    timeout=15,
                )
            except Exception as e:
