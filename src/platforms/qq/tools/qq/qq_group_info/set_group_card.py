...
async def _find_self_member(qq_client: Any, group_id: str, bot_id: str) -> dict[str, Any] | None:
    member_list = await qq_client.send_api(
...
async def _sync_confirmed_card(
    *,
    ...
    try:
        remote_group = await qq_client.send_api(
            "get_group_info",
            {"group_id": _onebot_id(group_id), "no_cache": True},
            timeout=8,
        )
...
async def _set_confirm_and_sync(
    *,
    ...
    resp = await qq_client.send_api_raw(
        "set_group_card",
        {
    ...
    for attempt in range(_POLL_ATTEMPTS):
        member = await _find_self_member(qq_client, group_id, bot_id)
        if member:
...
def make_handler(qq_client: Any, session: Any) -> Callable:
    def execute(card: str | None = None, **kwargs) -> dict:
        ...
        bot_id = str(getattr(qq_client, "bot_id", "") or "").strip()
        if not bot_id:
