...
"""platforms.qq.handler — QQ adapter 消息处理集成
...
from platforms.qq import adapter as qq_adapter
from platforms.qq.adapter import (
...
def _qq_adapter_cfg() -> dict[str, Any]:
    runtime = _qq_runtime()
    if runtime is None:
        return {}
    return runtime.adapter_config
...
async def _handle_qq_adapter_message(event: dict, conversation_id: str) -> None:
    ...
    debug_xml = await qq_adapter.qq_adapter_event_to_debug_xml(event, bot_id=bot_id, timezone=app_state.TIMEZONE)
    await broadcast_debug_xml(debug_xml, event)

    qq_adapter_cfg = _qq_adapter_cfg()
    ...
    ctx_entry = await qq_adapter.qq_adapter_event_to_context(
    ...
    # 下载 / 展开 / 视觉预处理
    await qq_adapter.download_pending_images(ctx_entry)
    await qq_adapter.expand_forward_previews(ctx_entry, client)
...
async def _handle_qq_adapter_recall(event: dict) -> None:
    ...
    if notice_type == "group_recall":
        ...
        conv_id = make_session_key("group", group_id)
        operator_info = await _resolve_group_member_display_info(group_id, operator_id)
        recall_entry = qq_adapter.build_recall_notice_entry(
            event,
        ...
        conv_id = make_session_key("private", peer_id)
    ...
    if not context_updated and notice_type == "friend_recall":
        ...
        fallback_keys.extend(
            ...
            if session_key != conv_id and (
                session_key.startswith("qq:private:")
                or session_key.startswith("qq:temp:")
                or session_key.startswith("private_")
                or session_key.startswith("temp_")
            )
        )
...
async def _handle_qq_adapter_poke(event: dict) -> None:
    ...
    if group_id:
        ...
        conv_id = make_session_key("private", sender_id)
    ...
    # 响应范围过滤
    if group_id:
        if whitelist_rejection_reason(_qq_adapter_cfg(), "group", group_id):
            return
    else:
        if whitelist_rejection_reason(_qq_adapter_cfg(), "private", sender_id):
            return
...
async def _handle_qq_adapter_group_notice(event: dict) -> None:
    ...
    if notice_type not in _GROUP_SYSTEM_NOTICE_TYPES:
        return
    if _qq_adapter_cfg().get("debug_only", False):
        return
    ...
    note_entry = qq_adapter.build_group_notice_entry(
        event,
