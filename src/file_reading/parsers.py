"""Deterministic, non-executing document extraction for local files."""
...
class ParserMessages:
    ...
    ocr_required: str
    network_disabled: str


DEFAULT_MESSAGES = ParserMessages(
    unsupported_file_type="该文件类型不能直接读取。",
    ocr_required="所选 PDF 页面没有可提取的原生文字，需要 OCR；当前文件读取能力暂不支持 OCR。",
    network_disabled="network access is disabled while reading local files",
)
...
def detect_type(path: Path, *, messages: ParserMessages = DEFAULT_MESSAGES) -> str:
    ...
    if extension in {"docm", "xlsm", "pptm", "doc", "xls", "ppt"}:
        raise ParseContractError(
            ...
            details={"detected_file_type": extension},
        )
...
def _parse_pdf(
    ...
    if pages and empty_pages == len(pages):
        raise ParseContractError("ocr_required", messages.ocr_required)
...
def parse_document(
    ...
    selection: dict[str, Any] | None,
    *,
    messages: ParserMessages = DEFAULT_MESSAGES,
) -> dict[str, Any]:
    ...
    kind = detect_type(path, messages=messages)
    try:
        if kind == "text":
            return _parse_text(path, selection)
        if kind == "pdf":
            return _parse_pdf(path, selection, messages)
        if kind == "docx":
    ...
    raise ParseContractError("unsupported_file_type", messages.unsupported_file_type)


def parse_document_safe(
    path_value: str,
    selection: dict[str, Any] | None,
    messages: ParserMessages = DEFAULT_MESSAGES,
) -> dict[str, Any]:
