...
@workspace_bp.route("/api/computer", methods=["GET"])
...
@workspace_bp.route("/api/computer/config", methods=["PUT"])
async def workspace_config_save():
    data = await request.get_json(silent=True) or {}
    if not isinstance(data, dict):
        return jsonify({"ok": False, "error": "Agent 电脑配置必须是对象"}), 400
    try:
        ...
        if observed.path_locked and candidate.install_root.casefold() != current.install_root.casefold():
            raise WorkspaceControlError("Agent 电脑建成后安装路径已锁定；请先完全卸载")
...
@workspace_bp.route("/api/computer/directory-selections/<selection_id>", methods=["GET"])
...
@workspace_bp.route("/api/computer/jobs", methods=["POST"])
...
@workspace_bp.route("/api/computer/jobs/<job_id>", methods=["GET"])
