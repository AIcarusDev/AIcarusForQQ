...
class MaintenanceService:
    ...
    _LONG_TERM_DELETE_ORDER: tuple[str, ...] = (
        ...
        "MemoryStorylineRevisions",
        "MemoryStorylineRelations",
        "MemoryThreadStateRevisions",
        "MemoryThreadStates",
        "MemoryCandidateStorylines",
        "MemoryStorylineMemberRevisions",
        "MemoryStorylineMembers",
        "MemoryStorylines",
        "MemoryStorylineRuns",
        ...
        return [
            {
                ...
                "expected_confirmation": self.expected_confirmation(RESET_COGNITION),
                ...
                "preserves": ["聊天记录", "长期记忆", "实体资料", "配置与统计数据"],
                "backup": {
                    "created": False,
                    "kind": "none",
                    "description": "不删除持久化数据，因此不会创建数据库备份。",
                },
            },
            {
                ...
                "expected_confirmation": self.expected_confirmation(DELETE_LONG_TERM_MEMORY),
                "confirmation_required": True,
                "target": "长期记忆、语义关系与派生检索数据",
                "summary": "删除旧事件记忆、记忆图谱、向量/检索辅助数据和待归档任务。",
                "keeps": "保留聊天记录、会话、bot_turns、实体/群资料、目标和用量统计。",
                "effects": [
                    "删除事件、参与者、来源、向量与检索索引",
                    "删除规范实体、事件关系、故事线与摘要缓存",
                    "取消尚未完成的归档任务",
                    "重置进程内召回缓存并重新初始化记忆 schema 状态",
                ],
                "preserves": ["聊天与会话记录", "bot_turns", "实体与群资料", "目标与用量统计"],
                "backup": {
                    "created": True,
                    "kind": "sqlite",
                    "description": "执行删除前在数据库目录的 backups 子目录创建完整 SQLite 备份。",
                },
            },
            {
                "id": CLEAR_ALL_DATA,
                ...
                "expected_confirmation": self.expected_confirmation(CLEAR_ALL_DATA),
                "confirmation_required": True,
                "target": "AIcarus SQLite 业务数据库",
                "summary": "备份后清空整个 SQLite 数据库，并重建为空库 schema。",
                "keeps": "保留配置文件、日志、模型文件和数据库备份。",
                "effects": [
                    "停止当前认知运行时与未完成的归档任务",
                    "删除数据库中的会话、消息、记忆、统计与运行状态",
                    "重建空数据库 schema 并清理进程内数据缓存",
                    "Core 可用时以新的空状态重新启动主循环",
                ],
                "preserves": ["配置文件", "日志", "模型文件", "执行前创建的数据库备份"],
                "backup": {
                    "created": True,
                    "kind": "sqlite",
                    "description": "执行清空前在数据库目录的 backups 子目录创建完整 SQLite 备份。",
                },
            },
        ]
