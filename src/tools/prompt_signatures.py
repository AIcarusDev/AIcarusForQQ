...
def build_prompt_signature(declaration: dict[str, Any]) -> str:
    ...
    lines.append(f"{name}(args: {_schema_to_ts(parameters, 0)})")
    return "\n".join(lines)
...
def _schema_to_ts(schema: dict[str, Any], indent: int) -> str:
    ...
    if isinstance(schema_type, list):
        return " | ".join(_schema_to_ts({"type": item}, indent) for item in schema_type)
    ...
    if schema_type == "array":
        ...
        if isinstance(item_schema, dict):
            item_type = _schema_to_ts(item_schema, indent)
        else:
            item_type = "unknown"
    ...
    if schema_type == "object" or "properties" in schema:
        return _object_to_ts(schema, indent)

    for key in ("anyOf", "oneOf"):
        variants = schema.get(key)
        if isinstance(variants, list) and variants:
            return " | ".join(
                _schema_to_ts(item, indent)
...
def _object_to_ts(schema: dict[str, Any], indent: int) -> str:
    ...
    for key, child_schema in props.items():
        ...
        field_type = _schema_to_ts(child_schema, indent + 1)
