...
这类工具由执行器按模型输出顺序串行执行，避免多个外部动作和其它工具并行交错。焦点切换工具（例如 `enter_qq_session`）与外界可感知工具同轮出现时，也遵循模型输出顺序；如果焦点切换发生在发送/撤回/戳一戳等动作之前，后续外界可感知工具会在真正执行前经过守门判断。启用 `tool_execution_guard` 后，如果工具真正执行前的 `<world>` 相比模型决策帧发生语义变化，这类工具还会先经过一次执行前守门判断；一旦某个外界可感知工具被守门拒绝，本轮后续外界可感知工具也会跳过并要求重新决策，但外界不可感知工具不受影响。
...
平台 manifest 可以给 namespace 声明 `activation.platform`。当前工具可见性只看
“当前打开的根平台”是否匹配这个字段；同一时间最多只有一个根平台打开，因此
QQ 平台 namespace 与 Core 平台 namespace 天然互斥。

`activation.surface` / `activation.surfaces` 暂时只作为未来平台内分层的保留字段，
构建工具集合时不生效。恢复 home/session 等平台内分层前，不要依赖这些字段摘除
namespace。
