...
这类工具由执行器优先按模型输出顺序串行执行，避免多个外部动作和其它工具并行交错。它们与 `shift` 同轮调用时会被阻断；当前焦点切换后再发起外部动作，应由下一轮重新决定。启用 `tool_execution_guard` 后，如果工具真正执行前的 `<world>` 相比模型决策帧发生语义变化，这类工具还会先经过一次执行前守门判断。
