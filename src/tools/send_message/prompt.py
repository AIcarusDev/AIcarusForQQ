...
建议：
  ...
  - 避免在同一条消息中同时包含表情包和文字，表情包应单独作为一条消息发送，效果更好。
...
反例 — 把表情包和文字塞进同一条消息（错误，应拆成两条分别发送）：
```json
{
  "messages": [
    {
      "segments": [
        { "command": "text",   "params": { "content": "哈哈哈" } },
        { "command": "sticker", "params": { "id": "xxx" } }
      ]
    }
  ]
}
```
*这样会把文字和表情包拼在同一条消息里一起发出，效果较差；应将它们拆成两条独立消息*×
"""