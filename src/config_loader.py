...
_DEFAULT_SOCIAL_TIPS_TEMP = """\
## 你当前在一个私聊临时会话中

这里有一些相关的提醒：

- 对方还**不是你的好友**，这是临时会话，注意安全和隐私。
...
- 避免长篇大论，对于大部分情况，每条消息应该简短、自然。但是注意聊天记录观察上下文，避免简短到让对方听不懂你想表达的意思。
...
- 在你已经拆分了多条消息的情况下，每条消息可以更加简短。标点符号也可以选择省略。
...
_PROMPT_DOC_DEFAULTS: dict[str, tuple[str, str]] = {
    ...
    "social_tips_temp": (
        os.path.join("config", "social_tips", "temp.md"),
        _DEFAULT_SOCIAL_TIPS_TEMP,
    ),
}
...
_PROMPT_DOC_SAVE_PATHS: dict[str, str] = {
    ...
    "social_tips_temp":    os.path.join(_CONFIG_DIR, "social_tips", "temp.md"),
}
